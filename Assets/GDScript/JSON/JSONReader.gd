...
static func read_json(s_path: String):
...
static func get_json_entry(dict_json: Dictionary, s_entry: String, s_path: String, v_default):
	# if dict_json does not contain the key "s_entry"
	if not dict_json.has(s_entry):
		#push_warning("entry " + s_entry + " not found at " + s_path)
		# return v_default
		return v_default
